...
api.get("/users", async (req, res) => {
  let usersArray = await users.find({}).toArray(); //truy vấn đến csdl
  let data = { users: [] };
...
api.get("/users/*/posts", async (req, res) => {
  ...
  //tìm kiếm tất cả các bài đăng về người dùng có id là userId
  let postArray = await posts.find({ userId: userId }).toArray();
  //duyệt qua mỗi bài đăng của người dùng và thêm vào data.posts
  ...
  //lấy bài đăng mà người dùng đang theo dõi
  ...
  //sắp xếp mảng data.posts theo thời gian giảm dần
...
api.get("/users/*", async (req, res) => {
  let userId = req.url.split("/")[2];
  let user = await users.find({ id: userId }).toArray(); //Truy vấn cơ sở dữ liệu để tìm người dùng có id là userId
  if (user.length !== 1) {
    res.status(404).json({ error: `No user with ID ${userId}` });
    return;
  }
...
api.post("/users", async (req, res) => {
  //kiểm tra có id hay id là rỗng hay ko
  ...
  //khởi tạo 'data' chứa thông tin người dùng mới
  let data = {
    id: newUserId,
    name: newUserName,
  ...
  delete data._id;
  res.json(data); //trả về thông tin người dùng mới dưới dạng JSON
});
...
api.post("/users/*/feed", async (req, res) => {
  ...
  if (!req.body.text || req.body.text.length === 0) {
    res.status(400).json({
      error:
        "the request body is missing a text property or the text is empty"
    });
    return;
  }
...
api.post("/users/*/follow", async (req, res) => {
  let userId = req.url.split("/")[2];
  let user = await users.find({ id: userId }).toArray();
  let targetId = req.query.target; //id của người dùng muốn theo dõi
  ...
  //kiểm tra id người dùng hiện tại có trùng với id người muốn theo dõi hay không
  if (userId === targetId) {
  ...
  //thêm id người dùng muốn theo dõi vào mảng following
